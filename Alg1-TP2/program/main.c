...
int main(int argc, char const *argv[]) {
  ...
  for( int i=0; i<n; i++ ){ //Faz leitura das coordenadas das cidades
    double a, b;
    assert(fscanf(cidades, "%lf %lf", &a, &b)==2) ;
    p[i].coordX = a;
    p[i].coordY = b;
  ...
  grafo = geraGrafo(p, n); //grafo é agora uma matriz de adjacencia com os pesos da arestas
  

  aresta *edge = criaVetorArestas(grafo, n); //edge agora é um vetor de arestas com o id das duas pontas e o peso da aresta
  int result; 
